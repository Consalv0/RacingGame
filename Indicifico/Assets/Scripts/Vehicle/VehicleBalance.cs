	...
	public class VehicleBalance : MonoBehaviour {
		...
		void ApplyLean() {
			if (vp.groundedWheels > 0) {
				...
				inverseWorldUp = vp.norm.InverseTransformDirection(Vector3.Dot(vp.wheelNormalAverage, Vector3.up) <= 0 ? vp.wheelNormalAverage : Vector3.Lerp(Vector3.up, vp.wheelNormalAverage, Mathf.Abs(Vector3.Dot(vp.norm.up, Vector3.up)) * 2));
				...
				targetLean = new Vector3(
					Mathf.Lerp(inverseWorldUp.x, Mathf.Clamp(-vp.rollInput * leanFactor.z * leanRollCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) + Mathf.Clamp(vp.localVelocity.x * slideLeanFactor, -leanFactor.z * slideLeanFactor, leanFactor.z * slideLeanFactor), -leanFactor.z, leanFactor.z), Mathf.Max(Mathf.Abs(Mathf.Max(Mathf.Abs(vp.steerInput), Mathf.Abs(vp.rollInput))))),
					Mathf.Pow(Mathf.Abs(actualPitchInput), pitchExponent) * Mathf.Sign(actualPitchInput) * leanFactor.x,
					inverseWorldUp.z * (1 - Mathf.Abs(Mathf.Max(Mathf.Abs(actualPitchInput * leanFactor.x), Mathf.Abs(vp.rollInput * leanFactor.z)))));
			} else {
				targetLean = vp.upDir;
			}

			//Transform targetLean to world space
			targetLeanActual = Vector3.Lerp(targetLeanActual, vp.norm.TransformDirection(targetLean), (1 - leanSmoothness) * Time.fixedDeltaTime).normalized;
