...
namespace RVP {
	...
	public class Wheel : MonoBehaviour {
		...
		void Start() {
			tr = transform;
			rb = tr.GetComponentInParent<Rigidbody>();
			vp = tr.GetComponentInParent<VehicleParent>();
			...
			if (tr.childCount > 0) {
				...
				if (Application.isPlaying) {
					...
					if (generateHardCollider) {
						...
						sphereCol.material = suspensionParent.frictionLessMaterial;
					}
		...
		void FixedUpdate() {
			...
			if (connected) {
				...
				if (vp.burnout > 0 && targetDrive.rpm != 0 && actualEbrake * vp.ebrakeInput == 0 && connected && grounded) {
					rb.AddForceAtPosition(suspensionParent.forwardDir * -suspensionParent.flippedSideFactor * (vp.steerInput * vp.burnoutSpin * currentRPM * Mathf.Min(0.1f, targetDrive.torque) * 0.001f) * vp.burnout * contactPoint.surfaceFriction, suspensionParent.tr.position, vp.wheelForceMode);
				}
				...
				if (airLeakTime >= 0) {
					...
					if (grounded) {
						airLeakTime += Mathf.Max(Mathf.Abs(currentRPM) * 0.001f, localVel.magnitude * 0.1f) * Time.fixedDeltaTime;
		...
		void GetWheelContact() {
			...
			RaycastHit[] wheelHits = Physics.RaycastAll(suspensionParent.maxCompressPoint, suspensionParent.springDirection, castDist, gameObject.layer);
			RaycastHit hit;
			int hitIndex = 0;
			bool validHit = false;
			...
			if (grounded) {
				...
				frictionForce = Vector3.Lerp(frictionForce,
						tr.TransformDirection(
							...
							, 0
							, sidewaysFrictionCurve.Evaluate(Mathf.Abs(sidewaysSlipFactor)) * -System.Math.Sign(sidewaysSlip) * sidewaysFriction * sidewaysSlipDependenceFactor * normalFrictionCurve.Evaluate(Mathf.Clamp01(Vector3.Dot(contactPoint.normal, Vector3.up))) * (vp.burnout > 0 && Mathf.Abs(targetDrive.rpm) != 0 && actualEbrake * vp.ebrakeInput == 0 && grounded ? (1 - vp.burnout) * (1 - Mathf.Abs(vp.accelInput)) : 1))
		...
		void OnDrawGizmosSelected() {
			...
			Gizmos.color = tirePressure == 0 ? Color.green : Color.cyan;
