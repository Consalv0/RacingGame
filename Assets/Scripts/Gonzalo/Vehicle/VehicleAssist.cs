	...
	public class VehicleAssist : MonoBehaviour {
		...
		void ApplySpinAssist() {
			...
			if (autoSteerDrift) {
				...
				targetTurnSpeed = vp.steerInput * driftSpinSpeed * (vp.localVelocity.z < 0 ? (vp.accelAxisIsBrake ? Mathf.Sign(vp.accelInput) : Mathf.Sign(Mathf.Max(Mathf.Abs(vp.accelInput), Mathf.Abs(-vp.brakeInput)))) : 1);
			}
		...
		void RollOver() {
			...
			if (vp.groundedWheels == 0 && vp.velMag < rollSpeedThreshold && vp.upDot < 0.8 && rollCheckDistance > 0) {
				if (Physics.Raycast(tr.position, vp.upDir, out rollHit, rollCheckDistance, gameObject.layer) || Physics.Raycast(tr.position, vp.rightDir, out rollHit, rollCheckDistance, gameObject.layer)) {
					rolledOver = true;
				} else {
					rolledOver = false;
				}
