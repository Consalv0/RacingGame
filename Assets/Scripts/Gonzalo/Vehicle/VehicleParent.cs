...
namespace RVP {
	...
	public class VehicleParent : MonoBehaviour {
		...
		[System.NonSerialized]
		public float forwardDot;//Dot product between forwardDir and Vector3.up
		[System.NonSerialized]
		public float rightDot;//Dot product between rightDir and Vector3.up
		[System.NonSerialized]
		public float upDot;//Dot product between upDir and Vector3.up
		[System.NonSerialized]
		...
		void OnCollisionEnter(Collision col) {
			if (col.contacts.Length > 0 && groundedWheels == 0) {
				foreach (ContactPoint curCol in col.contacts) {
					if (curCol.thisCollider.gameObject.layer != gameObject.layer) {
